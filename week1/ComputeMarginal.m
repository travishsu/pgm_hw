...
%
%   M is a factor containing the marginal over variables V, M = P()
...
p = ComputeJointDistribution(F);
x = ObserveEvidence(p,E);
y = FactorMarginalization(x,setdiff(x.var,V));
M = y;
M.val = y.val/(sum(y.val));
M = M
